...
DATA.heroBaseCost = 1700;   // first extra hero
...
DATA.units = {
  ...
  swordsman: {
    key: 'swordsman', name: 'Swordsman', cost: 70, hp: 140, dmg: 15, range: 2.3, cd: 1.0, armor: 0.15, speed: 5.6, radius: 0.42,
    attack: 'melee', desc: 'Reliable front-line fighter.',
    color: 0x8a97a5, cloth: 0x355d9a, weapon: 'sword', helmet: true, shield: true,
  },
  archer: {
    key: 'archer', name: 'Archer', cost: 80, hp: 85, dmg: 13, range: 17, cd: 1.3, armor: 0.05, speed: 5.6, radius: 0.4,
  ...
  engineer: {
    key: 'engineer', name: 'Engineer', cost: 100, hp: 100, dmg: 6, range: 2.0, cd: 1.2, armor: 0.05, speed: 5.8, radius: 0.4,
  ...
  crossbowman: {
    key: 'crossbowman', name: 'Crossbowman', cost: 110, hp: 95, dmg: 23, range: 15, cd: 1.7, armor: 0.1, speed: 5.4, radius: 0.4,
  ...
  cavalry: {
    key: 'cavalry', name: 'Knight', cost: 180, hp: 230, dmg: 28, range: 2.6, cd: 1.0, armor: 0.3, speed: 8.4, radius: 0.55,
  ...
  priest: {
    key: 'priest', name: 'Priest', cost: 130, hp: 85, dmg: 5, range: 2.0, cd: 1.5, armor: 0, speed: 5.6, radius: 0.4,
  ...
  apprentice: {
    key: 'apprentice', name: 'Apprentice', cost: 130, hp: 80, dmg: 22, range: 16, cd: 1.8, armor: 0, speed: 5.4, radius: 0.4,
  ...
  pikeman: {
    key: 'pikeman', name: 'Pikeman', cost: 110, hp: 180, dmg: 28, range: 3.3, cd: 1.5, armor: 0.2, speed: 5.0, radius: 0.45,
...
DATA.eliteChance = (n) => n < 5 ? 0 : Math.min(0.45, 0.03 + n * 0.01);
...
// ---------------------------------------------------------------------------
DATA.buildings = {
  wall:     { key: 'wall', name: 'Stone Wall', cost: 55, hp: 520, w: 1, d: 1, cat: 'defense', drag: true,
              desc: 'Sturdy stone. Drag to build long stretches. Enemies hack at walls twice as hard as at anything else.' },
  gate:     { key: 'gate', name: 'Gate', cost: 110, hp: 500, w: 1, d: 1, cat: 'defense', gate: true,
              desc: 'Your units pass through; enemies must break it.' },
  arrow_tower:   { key: 'arrow_tower', name: 'Arrow Tower', cost: 230, hp: 900, w: 2, d: 2, cat: 'tower', tower: true,
                   range: 20, dmg: 18, cd: 1.1, projectile: 'arrow', desc: 'Fast-firing tower. Good all-rounder.' },
  ballista_tower:{ key: 'ballista_tower', name: 'Ballista Tower', cost: 410, hp: 1000, w: 2, d: 2, cat: 'tower', tower: true,
                   range: 28, dmg: 86, cd: 2.8, projectile: 'ballista', bonusVsLarge: 1.7, desc: 'Slow, huge single hits. +70% damage to large enemies.' },
  mage_tower:    { key: 'mage_tower', name: 'Mage Tower', cost: 550, hp: 800, w: 2, d: 2, cat: 'tower', tower: true,
                   range: 22, dmg: 45, cd: 2.2, projectile: 'magic', desc: 'Lobs exploding fire that splashes in 3.5m and burns.' },
  frost_tower:   { key: 'frost_tower', name: 'Frost Tower', cost: 450, hp: 800, w: 2, d: 2, cat: 'tower', tower: true,
                   range: 18, dmg: 14, cd: 0.9, projectile: 'frost', desc: 'Weak damage but slows targets by 50%.' },
  cannon_tower:  { key: 'cannon_tower', name: 'Cannon Tower', cost: 670, hp: 1100, w: 2, d: 2, cat: 'tower', tower: true,
  ...
  lightning_tower:{ key: 'lightning_tower', name: 'Lightning Tower', cost: 590, hp: 850, w: 2, d: 2, cat: 'tower', tower: true,
                   range: 18, dmg: 28, cd: 1.7, chain: { count: 4, radius: 6 }, desc: 'Chain lightning that arcs to four enemies and ignores armour.' },
  poison_tower:  { key: 'poison_tower', name: 'Poison Tower', cost: 410, hp: 800, w: 2, d: 2, cat: 'tower', tower: true,
                   range: 18, dmg: 8, cd: 1.1, projectile: 'poison', desc: 'Darts that poison for 12 damage/s over 4s. Stacks nothing, but never misses.' },
  watchtower:    { key: 'watchtower', name: 'Watchtower', cost: 550, hp: 900, w: 2, d: 2, cat: 'tower', tower: true,
  ...
  barricade: { key: 'barricade', name: 'Spiked Barricade', cost: 35, hp: 240, w: 1, d: 1, cat: 'defense', drag: true, spikes: 3,
              desc: 'Cheap and weak, but every melee blow against it wounds the attacker for 3.' },
  trap:     { key: 'trap', name: 'Spike Trap', cost: 65, hp: 200, w: 1, d: 1, cat: 'defense', interior: [[0, 0]], trap: { dmg: 70, radius: 1.6, slow: 0.5, rearm: 20 },
              desc: 'Hidden in the ground. Enemies stepping on it take 70 damage and are slowed. Rearms after 20s.' },
  barracks: { key: 'barracks', name: 'Barracks', cost: 220, hp: 1200, w: 3, d: 3, cat: 'economy', soldierCap: 6,
              desc: '+6 soldier capacity. Recruits muster here.' },
  farm:     { key: 'farm', name: 'Farm', cost: 210, hp: 500, w: 3, d: 3, cat: 'economy', income: 45, costGrowth: 1.3,
              desc: '+45 gold at the end of each wave. Each farm you own makes the next one 30% pricier.' },
  mine:     { key: 'mine', name: 'Gold Mine', cost: 360, hp: 700, w: 2, d: 2, cat: 'economy', income: 90, costGrowth: 1.6,
              desc: '+90 gold at the end of each wave. Each mine you own makes the next one 60% pricier.' },
  blacksmith: { key: 'blacksmith', name: 'Blacksmith', cost: 410, hp: 900, w: 3, d: 3, cat: 'economy', unique: true, soldierDmg: 0.15,
              desc: 'Soldiers deal +15% damage. Unlocks the Weapon and Armor upgrades.' },
  shrine:   { key: 'shrine', name: 'Healing Shrine', cost: 300, hp: 600, w: 2, d: 2, cat: 'economy', heal: { radius: 12, hps: 10 },
              desc: 'During a wave, heals friendly units within 12m for 10 HP/s. Everyone is healed fully between waves anyway.' },
  market:   { key: 'market', name: 'Market', cost: 480, hp: 700, w: 3, d: 3, cat: 'economy', unique: true, killBonus: 0.25,
              desc: '+25% gold from kills. One per fortress.' },
  tavern:   { key: 'tavern', name: 'Tavern', cost: 360, hp: 800, w: 3, d: 3, cat: 'economy', unique: true, soldierCap: 3, soldierSpeed: 0.12,
...
DATA.buildings.dragon_roost = { key: 'dragon_roost', name: 'Dragon Roost', cost: 3600, hp: 1800, w: 3, d: 3, cat: 'wonder', unique: true, guardian: { unit: 'tamedragon', rebuild: 2 },
...
DATA.buildings.titan_forge = { key: 'titan_forge', name: 'Titan Forge', cost: 4700, hp: 2400, w: 3, d: 3, cat: 'wonder', unique: true, guardian: { unit: 'irongolem', rebuild: 2 },
  desc: 'Forges an Iron Titan: 5500 HP, 50% armour, 110-damage cleaving blows. Slow, unstoppable. Reforged two waves after it falls.' };
DATA.buildings.arcane_spire = { key: 'arcane_spire', name: 'Arcane Spire', cost: 4100, hp: 1300, w: 2, d: 2, cat: 'wonder', unique: true, tower: true, range: 44, dmg: 220, cd: 14, storm: { count: 6, scatter: 7, splash: 5, delay: 1.0 },
...
DATA.buildings.sun_altar = { key: 'sun_altar', name: 'Sun Altar', cost: 3300, hp: 1300, w: 2, d: 2, cat: 'wonder', unique: true, tower: true, range: 38, dmg: 160, cd: 1, beam: { dps: 160 },
...
DATA.buildings.royal_treasury = { key: 'royal_treasury', name: 'Royal Treasury', cost: 2800, hp: 1600, w: 3, d: 3, cat: 'wonder', unique: true, interest: 0.06, interestCap: 600, bounty: 0.5,
  desc: 'Pays 6% interest on your gold after every wave (up to 600) and raises every bounty your forces collect by 50%.' };
DATA.buildings.storm_crown = { key: 'storm_crown', name: 'Crown of Storms', cost: 6900, hp: 1500, w: 2, d: 2, cat: 'wonder', unique: true, tower: true, range: 40, dmg: 150, cd: 3, chain: { count: 10, radius: 9 },
  desc: 'Chain lightning every 3s that leaps through ten enemies within 40m for 150 each, ignoring armour.' };
DATA.buildings.phoenix_pyre = { key: 'phoenix_pyre', name: 'Phoenix Pyre', cost: 9000, hp: 1800, w: 2, d: 2, cat: 'wonder', unique: true, phoenix: true,
  desc: 'Every soldier and hero of yours that falls rises again from the pyre three seconds later, once per wave.' };
DATA.buildings.world_tree = { key: 'world_tree', name: 'World Tree', cost: 11000, hp: 3000, w: 3, d: 3, cat: 'wonder', unique: true, income: 150, treeAura: { radius: 32, regen: 0.02, towerDmg: 1.25 },
...
DATA.buildings.time_anchor = { key: 'time_anchor', name: 'Time Anchor', cost: 14000, hp: 1800, w: 2, d: 2, cat: 'wonder', unique: true, slowField: { radius: 34, factor: 0.45 }, freeze: { every: 30, radius: 20, dur: 4 },
  desc: 'Enemies within 34m move and fight 45% slower. Every 30s it freezes everything within 20m for 4s.' };
DATA.buildings.celestial_gate = { key: 'celestial_gate', name: 'Celestial Gate', cost: 20000, hp: 2400, w: 3, d: 3, cat: 'wonder', unique: true, summonHost: { unit: 'angel', count: 6, every: 40, dur: 40 },
...
DATA.buildings.doomsday_engine = { key: 'doomsday_engine', name: 'Doomsday Engine', cost: 28000, hp: 3000, w: 3, d: 3, cat: 'wonder', unique: true, doom: { every: 60, dmg: 450, bossDmg: 220 }, burnField: { radius: 36, dps: 20 },
...
DATA.buildings.throne_of_ages = { key: 'throne_of_ages', name: 'Throne of Ages', cost: 41000, hp: 5000, w: 3, d: 3, cat: 'wonder', unique: true, throne: { kingHp: 3000, kingRegen: 60, towerDmg: 1.35, soldierDmg: 1.35, income: 1.5 },
...
DATA.buildings.sun_forge = { key: 'sun_forge', name: 'Solar Forge', cost: 62000, hp: 4000, w: 3, d: 3, cat: 'wonder', unique: true, towerBoost: { cd: 0.67, range: 1.25 }, globalStats: true,
  desc: 'The sun itself is your smith: every tower of yours fires half again as fast and reaches 25% further.' };
DATA.buildings.comet_shrine = { key: 'comet_shrine', name: 'Comet Shrine', cost: 90000, hp: 4000, w: 3, d: 3, cat: 'wonder', unique: true, comet: { every: 50, dmg: 2400, radius: 11 }, meteorRain: { every: 6, dmg: 280, splash: 4 },
...
DATA.buildings.heart_of_winter = { key: 'heart_of_winter', name: 'Heart of Winter', cost: 124000, hp: 5000, w: 3, d: 3, cat: 'wonder', unique: true, slowField: { radius: 70, factor: 0.35 }, brittle: 1.15, freeze: { every: 45, radius: 60, dur: 4 }, globalStats: true,
...
DATA.buildings.apotheosis = { key: 'apotheosis', name: 'The Apotheosis', cost: 520000, hp: 20000, w: 3, d: 3, cat: 'wonder', unique: true, apotheosis: { burn: 0.04, bossBurn: 0.015, radius: 60, skyEvery: 45, sky: 0.4, bossSky: 0.15, bounty: 3, towerCd: 0.6, guard: 0.6 }, globalStats: true,
...
// ---------------------------------------------------------------------------
DATA.upgrades = {
  walls:    { key: 'walls', name: 'Reinforced Walls', cost: 210, max: 5, desc: 'Walls, gates and the Keep gain +40% HP per level.' },
  towers:   { key: 'towers', name: 'Tower Engineering', cost: 280, max: 5, desc: 'Towers deal +20% damage and gain +8% range per level.' },
  weapons:  { key: 'weapons', name: 'Weapon Smithing', cost: 250, max: 5, requires: 'blacksmith', desc: 'Soldiers deal +15% damage per level. Requires a Blacksmith.' },
  armor:    { key: 'armor', name: 'Plate Armor', cost: 250, max: 5, requires: 'blacksmith', desc: 'Soldiers take 8% less damage per level. Requires a Blacksmith.' },
  hero:     { key: 'hero', name: 'Hero Training', cost: 340, max: 5, desc: 'Heroes gain +15% damage and +15% HP per level.' },
  royal:    { key: 'royal', name: 'Royal Guard', cost: 300, max: 5, desc: 'The King gains +25% HP and +2 HP/s regen per level.' },
  garrison: { key: 'garrison', name: 'Garrison', cost: 160, max: 6, desc: '+3 soldier capacity per level.' },
  marksman: { key: 'marksman', name: 'Marksmanship', cost: 220, max: 4, desc: 'Archers, crossbowmen and apprentices gain +12% range and +10% damage per level.' },
  fortune:  { key: 'fortune', name: 'Fortune', cost: 410, max: 3, desc: '+10% gold from every source per level.' },
};
...
// ---------------------------------------------------------------------------
DATA.waves = {
  budget: (n) => 48 + 30 * n + 4 * n * n + 0.06 * n * n * n,     // wave 35 spends about 8000 reward-points on enemies, wave 60 about 27000
  clearBonus: (n) => 60 + 26 * n,
  hpScale: (n) => n <= 8 ? 1 : 1 + (n - 8) * 0.045,
  dmgScale: (n) => n <= 12 ? 1 : 1 + (n - 12) * 0.025,
